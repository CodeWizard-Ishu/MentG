...
export const getProfileData = async (req:any, res:any) => {
    ...
    try {
        ...
        const uniqueMentees = new Set(mentorProfile.bookings.map(booking => booking.menteeId)).size;
        const completedSessions = mentorProfile.bookings.length;

        // Calculate average rating
        const averageRating = mentorProfile.ratings.length > 0 
            ? (mentorProfile.ratings.reduce((acc, rating) => acc + rating.score, 0) / mentorProfile.ratings.length).toFixed(2)
            : 0;
        ...
        console.error(error);
        return res.status(500).json({ error: 'An error occurred while fetching mentor profile' });
    }
};